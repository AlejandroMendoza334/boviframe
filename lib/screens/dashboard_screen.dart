...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> _cargarEvaluaciones() async {
    ...
    for (final ses in sesionesSnapshot.docs) {
      final evals =
          await ses.reference.collection('evaluaciones_animales').get();
    ...
    for (var l in letras) {
      ...
      nuevosProm[l] =
          todosDatos.isEmpty ? 0.0 : suma / todosDatos.length;
    }

    // 2) Calcular “índice” y ordenar
    final listaIndice = todosDatos
        .map((animal) {
  ...
  Widget _buildAnimalCard(Map<String, dynamic> a) {
    ...
    return Card(
      ...
      child: ListTile(
        ...
        subtitle:
            Text('Índice: ${(a['indice'] as double).toStringAsFixed(1)}'),
        onTap: () =>
  ...
  BarChart _buildBarChart() {
    const letras = ['E', 'P', 'M', 'U', 'R', 'A', 'S'];
    final valores =
        letras.map((l) => promedios[l] ?? 0.0).toList();
    final maxY =
        ((valores.isEmpty ? 0.0 : valores.reduce(max)) + 2).toDouble();
    ...
    return BarChart(BarChartData(
      ...
      barGroups: List.generate(valores.length, (i) {
        return BarChartGroupData(x: i, showingTooltipIndicators: [0], barRods: [
          BarChartRodData(
            toY: valores[i],
            width: 12,
